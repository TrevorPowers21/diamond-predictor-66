...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const findColPlus = (plusPatterns: string[], fallbackPatterns: string[]) => {
      // First try exact "+" column match (handle optional space before +)
      const plusMatch = keys.find((k) => plusPatterns.some((p) => k.replace(/\s+\+/, "+").trim() === p || k.trim() === p));
      if (plusMatch) return plusMatch;
      // Then fallback patterns
      return keys.find((k) => fallbackPatterns.some((p) => k.includes(p)));
    };
    const avgCol = findColPlus(["ba power rating+"], ["avg_power", "avg_pr", "avg_rating", "ba power rating"]);
    const obpCol = findColPlus(["obp power rating+"], ["obp_power", "obp_pr", "obp_rating", "obp power rating"]);
    const slgCol = findColPlus(["slg power rating+"], ["slg_power", "slg_pr", "slg_rating", "slg power rating"]);
