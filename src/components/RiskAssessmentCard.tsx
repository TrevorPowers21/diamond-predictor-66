        ...
        {risk.factors.map((f) => (
          <div key={f.label}>
            <div className="flex items-center gap-2">
              ...
              <div className="flex-1 h-2.5 rounded-full bg-[#162241] overflow-hidden">
                <div
              ...
              <div className={cn("w-8 text-[9px] font-semibold", f.score <= 25 ? "text-[hsl(142,71%,35%)]" : f.score <= 50 ? "text-[hsl(200,80%,35%)]" : f.score <= 75 ? "text-[hsl(40,90%,38%)]" : "text-[hsl(0,72%,41%)]")}>
                {scoreLabel(f.score)}
              </div>
            </div>
            <div className="ml-[80px] text-[10px] text-[#6b7280] leading-tight mt-0.5">{f.detail}</div>
          </div>
        ))}
      </div>
    </div>
