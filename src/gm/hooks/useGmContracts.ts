...
import { syncContractFunding, type FundingMode } from "@/gm/lib/contractFundingSync";
...
export function useGmContracts(playerId?: string | null) {
  ...
  const addContract = useMutation({
    mutationFn: async (input: NewContractInput) => {
      ...
      const allocationId = await syncContractFunding(supabase, effectiveTeamId, user?.id ?? null, {
  ...
  const removeContract = useMutation({
    mutationFn: async (c: GmContract) => {
      ...
      if (effectiveTeamId) await syncContractFunding(supabase, effectiveTeamId, user?.id ?? null, {
