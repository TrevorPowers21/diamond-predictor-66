...
export function useTeamBuilderData({
  ...
  const { data: allPlayersForSearch = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      while (true) {
        const { data, error } = await supabase
          .from("players")
          .select("id, first_name, last_name, position, is_twp, class_year, throws_hand, bats_hand, team, from_team, conference, transfer_portal, portal_status, player_predictions(id, customer_team_id, from_avg, from_obp, from_slg, from_era, from_fip, from_whip, from_k9, from_bb9, from_hr9, p_avg, p_obp, p_slg, p_ops, p_iso, p_wrc_plus, p_era, p_fip, p_whip, p_k9, p_bb9, p_hr9, p_rv_plus, pitcher_role, power_rating_plus, class_transition, dev_aggressiveness, model_type, status, variant, updated_at, o_war, p_war, market_value, twp_hitter_market_value, twp_pitcher_market_value, projected_ip, projected_pa, hitter_depth_role, pitcher_depth_role), nil_valuations(estimated_value, component_breakdown)")
  ...
  const { data: returners = [], dataUpdatedAt: returnersUpdatedAt } = useQuery({
    ...
    queryFn: async () => {
      ...
      const selectCols =
        "id, first_name, last_name, position, is_twp, class_year, throws_hand, bats_hand, team, from_team, conference, transfer_portal, source_player_id, portal_status, player_predictions(id, from_avg, from_obp, from_slg, from_era, from_fip, from_whip, from_k9, from_bb9, from_hr9, p_avg, p_obp, p_slg, p_ops, p_iso, p_wrc, p_wrc_plus, p_era, p_fip, p_whip, p_k9, p_bb9, p_hr9, p_rv_plus, pitcher_role, power_rating_plus, class_transition, dev_aggressiveness, model_type, status, variant, updated_at, o_war, p_war, market_value, twp_hitter_market_value, twp_pitcher_market_value, projected_ip, projected_pa, hitter_depth_role, pitcher_depth_role)";
