...
**RESOLVED 2026-08-21 (commit 3241608, dry-run verified):** hitter env+ → stored ba/obp/iso_plus (no live compute; stored ≈ old live Δ≤3); hitter from-team → id-first (team_id/source_team_id/name); pitcher batch overlays model_config weights (single source, legacy Equation Weights table retired). Dry-run Georgia: hitter 4986 computed (96%), pitcher 5059 computed (97%, "overlaid 16 weights"), JUCO correctly blocked (no_from_conf). Both engines now consistent + model_config-sourced. Ready for the real re-run (Step 2).

---
## STEP 2 — TRANSFER RE-RUN (running 2026-08-21)
Recomputing all transfer projections on staging with the new STORED env+ (ratio) + re-tuned weights, for all 17 customer teams × hitter + pitcher (`scripts/_run_step2_all.sh`). Writes `player_predictions` transfer/precomputed/2027. Verified pre-run: equation pulls stored env+ (both sides), resolves teams by source_team_id (team_id empty — backfill deferred, Trevor "rock with source id for now"), power-rating + competition + park levers all directionally correct (controlled tests). Prod: same loop via `:prod` after env+ cols + model_config weights land on prod (PROD_MIGRATIONS_TODO Step-2 entry).
**STILL BUILDING / NEXT:** verify big movers post-run; pipeline steps 3-6 (confirm conf stats + team_season_stats rows from pitch log; refresh team-build + player/transfer snapshots WITHOUT toggle changes; wire displays incl. Program Analytics; complete prod-migration doc); then the unified edge fn (Track B). Deferred: players.team_id backfill; HTP (100−wRC+)→park-factor swap (build before storing HTP); NCAA anchors → ncaa_averages; browser pitcher-weight loader → model_config; JUCO separate function.

---
## SNAPSHOT REFRESH DONE (Step 6, 2026-08-21)
After the Step-2 transfer re-run, refreshed all saved-build + target snapshots from the new projections WITHOUT changing toggles. Two-step: (1) `backfill-neutral-snapshot.ts --apply` → `neutral_snapshot` from current predictions (1174 build players + 167 targets; precomputed-transfer > regular-returner); (2) `heal-stale-snapshots.ts --apply` → re-derive `player_snapshot`/`transfer_snapshot = projectEffectiveWar(new neutral, production_notes)` (1271/1271 healed, 0 err). Toggles (`production_notes`) untouched. Covered all 38 builds incl. 15 default rosters. **⭐ Big WAR shifts for near-stable wRC+ = the ÷10→÷13.1 WAR recalibration flowing into saved builds** (not the transfer levers) — marginal players correctly drop below replacement. **This is the "automatic function" for snapshot refresh — must run on prod after the prod transfer re-run** (PROD_MIGRATIONS_TODO Step-6 entry). NEXT: pipeline steps 3-5 (confirm conf stats + team_season_stats from pitch log; wire displays incl. Program Analytics team snapshots), deploy edge fn, then unified edge fn.
...
---
## ★★★★ VERY IMPORTANT — CONF-STATS PRODUCERS TO CODIFY BEFORE PROD (2026-08-21) ★★★★
Conference Stats FEEDS transfers/HTP/team_season_stats/Program Analytics. Some columns were hand-run SQL on staging → **will be EMPTY on prod unless committed producers exist.** NOW committed: run_env_factor + OPR + canonical park-swap HTP (`scripts/derive_conf_opr_htp.ts`). **STILL TODO (must commit): the raw-rate pitch-log Bucket-A assembly + WRC_plus** (currently a commented-out SQL, `scripts/sql/conf_stats_unified_assembly.sql`). If not codified, transfers + HTP + Program Analytics break silently on prod. Runbook "CRITICAL PROD-PUSH BLOCKER"; map: `docs/CONFERENCE_STATS_BUILD_PROCESS_2026_08_21.md`. End state = ONE edge-fn conf-stats-derive step (Track B).

---
## FULL PIPELINE WALKTHROUGH (ingest → snapshot) — `docs/FULL_PIPELINE_WALKTHROUGH_2026_08_21.md`
The 12 confirmed steps (pitch-log ingest → derive Masters → Stuff+ → ncaa_averages → scores/power ratings → std_pr → create_predictions → returner recompute → CONFERENCE-STATS build (9a-9h) → TRANSFER projections → team_season_stats → SNAPSHOT refresh), each = a module of the unified edge fn (Track B). All downstream reads STORED (no live compute); teams by ID; snapshots team-scoped + toggle-preserving. Prod blockers: 9a raw-rate assembly + 9f WRC_plus still hand-run SQL. What's-next: codify those 2, display-wiring audit, **evaluate/redo the MARKET VALUE equation (Trevor)**, deploy edge fn, unify into one edge fn, prod push. HTP % impact confirmed no re-tune (weight 0.28 already calibrated to park-swap SD 14.31 = 4.01%). Snapshots re-refreshed after the HTP re-run (29 pitcher rows).
