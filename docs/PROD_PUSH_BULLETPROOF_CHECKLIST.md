...
> **ROLE OF THIS FILE:** what must be TRUE before and after each step — the gate definitions and their measured
> values. **It is not the step list and not the build spec.**
> · **Step list** → `docs/PROD_PUSH_STEPS_2026_08_26.md` · **Build spec** → the CANONICAL BUILD SPEC in `PIPELINE_pitch_log_to_projections.md`
> · **Branch-wide readiness** → `docs/PROD_PUSH_BULLETPROOF_CHECKLIST_BRANCHWIDE.md` · **State** → `docs/HANDOFF_RESUME_2026_08_31_SNAPSHOTS.md`
> 🛑 **THE GATE DOCTRINE (a count gate caught NONE of the 28 findings):** gate on **VALUE · MEMBERSHIP · CARDINALITY ·
> IDENTITY · LOG-CONTENT · SIGN**. ⛔ **NEVER** on counts, exit codes, `updated_at`, presence of a config key, or a
> magnitude derived from the model's own constants. ⛔ **Scope every gate to `division='D1'`** unless it tests JUCO.
> ✅ **2026-08-31: the full prod↔staging audit PASSED** — zero columns populated on staging but empty on prod; the
> descriptive WAR chain and `run_env_factor` are bit-identical. See "FULL PROD ↔ STAGING AUDIT".
...
**GATES THIS ADDS:**
1. `"Equation Weights"` is renamed to `_LEGACY_2025` (or empty) on the target DB.
2. A returner's `p_wrc_plus` reproduces `((.011 + .691·OBP + .235·SLG) / .3782) × 100` within ±1.5 —
   **on prod today it does NOT** (23% match; the legacy 4-stat formula matches 100%).
3. `model_config` admin_ui @2026 is the ONLY config source any reader touches.
...
**GATES THIS ADDS (VALUE gates — never counts):**
1. `model_config` (admin_ui, season=CURRENT_SEASON) holds **41** calibration keys, not 19 — including
   `<key>_pr_center` + `<key>_pr_sd` for all 11 ratings.
2. `era_plus_ncaa_avg` reads **~5.264** (D1-only), NOT 5.483215 (all-division).
3. Projected D1 ERA mean lands within ~0.05 of the actual mean at the SAME qualifier — checked at
   **p05 / p10 / median / p90**, not the mean alone.
4. No projected HR9 < 0, and elite HR9 reaches ~0.35 (the 2026-08-24 holdout).
