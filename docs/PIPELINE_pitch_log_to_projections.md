...
| 5 | **Masters rollup** | `pitch_log_{pitcher,hitter}_totals` | `"Pitching Master".stuff_plus` **+ CREATE missing Master rows** | 🐛 **BLOCKED BY A CONFIRMED BUG — `derive_masters_from_pitchlog.ts` CANNOT create a hitter row.** `:465` passes `"batting_team_id"` where `repRows`' `idCol` belongs (the pitcher call at `:486` correctly passes `"pitcher_id"`), so it queries `pitch_log WHERE batting_team_id = <a player id>` — matches nothing, **exceeds the statement timeout** over 2.5M rows, and `:451` discards the `error`, so it silently skips EVERY hitter. **FIX: `:465` → `"batter_id"`, and stop swallowing the error at `:451`.** Gates are otherwise correct (`MIN_PA` 25 / `MIN_BF` 20 + D1). Creation must run BEFORE anything iterating the Masters, and be gated on the MEMBERSHIP query — `0 rows created` is indistinguishable from `nothing to create`. |
...
| 18 | **Projections** (returner + transfer) | Masters, `model_config`, `"Conference Stats"`, **`team_season_stats.faced_*`**, **`players.is_twp`** | `player_predictions` | ★ empty `team_season_stats` ⇒ Independents silently lose faced-competition. ★★ **EVERY one of the four projection paths MUST route TWP markets to `twp_hitter_market_value` / `twp_pitcher_market_value` and NULL the shared `market_value`** — the display layer reads the `twp_*` columns whenever `is_twp` is true and ignores `market_value` entirely, so a TWP priced into the shared column renders **BLANK**. The transfer HITTER engine was the ONLY one missing it (2,119 rows blank). ⛔ **Never repair this downstream** — the market must be priced at the **DESTINATION program's conference**, which only this stage knows. See REGISTRY #20. |
| 19 | **Composite WAR / markets / snapshots** | `player_predictions` | predictions, snapshots, `target_board` | after 18, never before. ★★ **THE BOARD REFRESH IS *ONE* STAGE:** re-copy WAR from predictions **then** re-price at the **build program's conference**, with **no committed state in between** — split across two steps a TWP board legitimately shows `$0`/blank (seen live, F41a→F42). ⛔ **NEVER re-derive a market here from the player's OWN conference** — only stage 18 knows the destination, and re-pricing locally under-prices by the full PTM ratio (measured **2.9×**). This stage RE-PRICES stored WAR; it does not re-decide the basis. See REGISTRY #19/#20.|
...
### ⚖️ WEIGHTING — **PER-ROW, BY DECISION** (settled 2026-09-01)

Stage 5.5 computes every anchor and every rating centre as a **PER-ROW mean** — each qualified pitcher
counts once, whether he threw 40 innings or 120. `twoSided()`:
`vals.reduce((a,b)=>a+b,0) / vals.length`.

**This is a CHOICE, not an oversight.** The two framings answer different questions:
| framing | answers | used for |
|---|---|---|
| **per-row** | *"the average PITCHER"* | **stage 5.5** — projecting an individual against other individuals |
| **IP/PA-weighted** | *"the average INNING/PA"* | conference & region baselines, run environment ([[feedback_weighted_region_averages]]) |

Volume is carried SEPARATELY by `projected_ip` via the depth role, so the rate baseline answers "how
well," not "how much." Mixing volume into the rate baseline would corrupt the first question.

**MEASURED GAP (PROD, D1, IP>=40):**
```
            per-row     IP-weighted   diff
ERA anchor   5.2635       5.1325      −2.5%
BB9 anchor   3.7734       3.6484      −3.3%
FIP anchor   4.4307       4.3603      −1.6%
era centre 109.725      111.413
bb9 centre 123.161      126.606
```

🛑 **THE RULE THAT MATTERS — NEVER MIX THE TWO.** The anchor and the centre MUST be computed with the
SAME expression on the SAME population. An IP-weighted centre against a per-row anchor (or vice versa)
reintroduces a constant offset of ~1.7 rating points ≈ **0.09 ERA** — i.e. it recreates C1 in miniature.
...
### ★★★ THE TEAM BUILDER READ/WRITE PATH — 2026-09-01 (the expensive night) ★★★

🛑 **ONE DEFECT CLASS BEHIND EVERY SYMPTOM: A STORED COPY NOBODY RECOMPUTES, BEHIND A `??` CHAIN THAT
SILENTLY CHANGES WHICH SOURCE WINS WHEN A FIELD BECOMES POPULATED.** Snapshots after a precompute.
Market after a WAR change. `transfer_snapshot` after a save. `p.prediction` degrading to a prediction
row. Each fix exposed the next because the CHAIN was the defect, not any single link.

#### 1. `p.prediction` IS NOT A SNAPSHOT — this cost hours
`useLoadBuild:411` → `activePred = snapshot ?? predictionMap[pid|side]`. It **degrades to the raw
prediction row whenever the snapshot lookup misses**, so every "fix" that read `p.prediction` was
still reading a prediction on exactly the rows that were wrong.
⇒ `useLoadBuild` now also exposes **`player_snapshot: rawSnapshot`**, and display reads
**`p.player_snapshot ?? p.transfer_snapshot`** — the two REAL snapshots, never `p.prediction`.

#### 2. FILLING A PREVIOUSLY-NULL FIELD FLIPPED THE WHOLE PAGE
`shown = p.neutralPrediction ?? p.prediction` worked ONLY because `neutral_snapshot` was mostly NULL.
Backfilling neutral onto all 1,254 rows made a dead branch live **for every row at once**, and every
toggled/depth-adjusted value regressed to the dev_agg=0 line.
⇒ **A `??` chain is not a precedence decision.** Order fallbacks by INTENT, not by what happens to be
populated. Display now reads the stored snapshot; **neutral is unreachable from display.**

#### 3. THE THREE GUARDRAILS (all required — removing any one re-opens compounding)
```
...
```
`PlayerProfile` has never had this bug because of #3: `devAggScale = isSnapshotBacked ? 1 : ratio`.
Team Builder computed the ratio unconditionally.
**Sequence:** toggle → dirty → scale neutral ONCE (the live bridge) → save bakes it → row clean →
stored read verbatim, never scaled again.
⛔ Gating the bridge off froze every toggle AND made the save persist the UNSCALED line.

#### 4. THE SAVE BAKES **NEUTRAL × THE TOGGLE** — never a re-read projection
`playerProjection()` returns the stored snapshot verbatim for a clean row, so if the row was clean at
serialize time the save wrote the UNSCALED line back. Measured: `production_notes.devAggressiveness
= 1` while `player_snapshot` held `.3172 / wRC+ 122 / dev_aggressiveness 0` — intent saved, effect
lost, so every reload showed neutral. ⇒ the save now calls
`playerProjection({ ...rp, _dirty: true })`, the same derivation
`scripts/rebake-player-snapshot-toggles.ts` performs offline, so a saved row and a re-baked row agree
BY CONSTRUCTION.

#### 5. EVERY LOCAL STATE UPDATE AFTER A SAVE MUST REFRESH **EVERY** SNAPSHOT COPY
...
the **stale** `player_snapshot` the instant `_dirty` cleared. That is the **flash up → back down →
correct after a DB round-trip**. Both copies must settle together.

#### 6. AN EFFECT WITH `exhaustive-deps` DISABLED CLOSES OVER STALE STATE
The auto-load effect re-runs whenever `buildsLoading` flips (ANY React Query refetch — window focus),
calls `loadBuild()`, and rebuilds `rosterPlayers`, **wiping `_dirty` and the unsaved toggle**. A guard
reading `rosterPlayers` directly there saw a STALE array and never fired. ⇒ guard via
**`dirtyRowsRef`**, kept current by its own effect. **A row mid-toggle is unsaved work; reloading over
it is data loss, not a refresh.**

#### 7. TWO STORED COPIES OF ONE PLAYER — roster vs board
39 of 462 active-build rows also had a `target_board` row for the same team, and they disagreed
(Hanley build 2.854/$285,391 vs board 2.331/$233,077). **THE RULE (Trevor):** players arrive on the
board with stored values and NO toggles; **once rostered, the board reads the roster's snapshot.**
`scripts/sync-board-from-roster-snapshot.ts` — side-aware (the ROSTER slot decides the side) and
TWP-aware. staging 32 · prod 47 · rostered-on-board still differing: **0**.
⚠ Board rows spell oWAR as **`owar`** and market as **`nil_valuation`**.

#### 8. SLOT IS AUTHORITATIVE FOR SIDE — not snapshot content
Kenny Ishikawa's SP row carried a HITTER neutral, so a content-based branch wrote hitter fields onto a
pitcher slot (wRC+ 118 / oWAR 1.187 / platoon_starter) competing with his real RF row (2.006) — which
is why his cornerstone role "never persisted". Branch on `position_slot`; null the opposite side.

#### 9. DEPTH ROLE DRIVES IP/PA — and MARKET IS STORED, NOT DERIVED
Neiswonger (weekend starter, 3.21 ERA / 2.82 FIP) showed **1.14 pWAR / $99k**: his row carried a
swing-starter `projected_ip` of 30. pWAR is linear in IP.
`pitcherExpectedIp(depthRole)` → `weekend_starter = pwar_ip_sp (85)` ⇒ **pWAR 3.329 · $332,852**.
`market_value` is a STORED snapshot field — nothing derives it at read time, so changing WAR without
rewriting market leaves a value from the OLD WAR.

#### 10. THREE FALSE ALARMS I RAISED — same root cause each time
Conclusions the evidence did not contain. (a) "sub-40-IP pitchers diverge between implementations" —
stale local rows vs fresh edge rows, two GENERATIONS not two implementations. (b) "local
`total_hitter_war` drifts" — exact on 221,318 rows; I measured LOCAL components against the EDGE
total. (c) "4 board rows wrong" — all Josiah Overbeek, a TWP whose pitcher-slot rows correctly hold
pWAR while `coalesce(o_war, p_war)` pulled his hitter oWAR.
⇒ **Before diffing two things, prove they are COMPARABLE: same generation (`updated_at`), same side (a
TWP carries both on ONE row), same field name.**

#### ⚠ STILL OPEN
- **10 pitchers staging / 18 prod** whose pWAR formula cannot be verified on their own neutral row — SKIPPED, not guessed
- **1 wrong-side neutral** (slot=pitcher, neutral is a hitter row) — reported, not patched over
- **JUCO PTM** (Hayden Blair) — known, parked
- **Removal from the roster** — nothing rewrites `transfer_snapshot` when a player comes OFF the roster; current behaviour is inertia, not design
- **THE DURABLE FIX**: one save path owning every derived copy together. Tonight's scripts are REPAIRS.
...
| scope | ONE team — **full program onboarding** (see correction below) | the WHOLE chain |
| stages | stage 18 **plus the default-roster build and GM init** | stages 1–19 |

### 🛑 MUST READ — CORRECTION (2026-08-31): "STAGE 18 ONLY" WAS **WRONG**
The two rows above previously read *"ONE team's transfer projections"* / *"**stage 18 ONLY**"*, and the line
below them read *"one stage, for one team — not a pipeline."* **That is a material understatement of what
onboarding does, and it caused this function to be scoped as a minor fragment when it is not.**
Trevor, 2026-08-31: *"part of what adding a new team is is basically running the precomputes into the program,
building a default roster for them and storing the neutral snapshots based on the projections."* **Correct.**

Verified against the source (`supabase/functions/process-precompute-jobs/index.ts`) — it writes **four** surfaces:

| # | write | line | what it is |
|---|---|---|---|
| 1 | `player_predictions` upsert (hitters) | 1274 | the precomputes |
| 2 | `player_predictions` upsert (pitchers) | 1658 | the precomputes |
| 3 | `team_builds` + `team_build_players` — **DELETEs the existing default build, then recreates it** | 1879–1905 | the **default roster + neutral snapshots** |
| 4 | `gm_budget`, `gm_activity` | 1930–1931 | GM (front-office) init |

⇒ It is **program onboarding**, not one stage. ⚠ Note #3 is **destructive-then-recreate**: it deletes the
team's existing default build before rebuilding it.

★ **WHY THIS MATTERS FOR THE DEPTH-ROLE ANCHOR (registry #9).** `team_build_players.player_snapshot` freezes
`o_war`, `p_war`, `market_value` and the depth role itself. The chain is:
`regular_season_pa/ip → depth role → projected PA/IP → oWAR/pWAR → market value → frozen into the snapshot`.
A mis-tiered role does not just mislabel a player — it **bakes a wrong WAR and a wrong market value into the
default roster the program sees on day one.**

⚠ **REGISTRY #9 IS IN THIS FUNCTION IN *THREE* PLACES, NOT TWO.** Fixed 2026-08-31: the hitter precompute
(~1214) and pitcher precompute (~1613) now anchor on `regular_season_pa ?? pa` / `regular_season_ip ?? IP ?? ip`,
**and the Master selects had to be widened first — `regular_season_*` was selected NOWHERE, and the Pitching
Master select did not even include `IP`.** ⬜ **STILL OPEN:** the default-build fallback at ~1836/1838
(`hitterDepthFromPa(p.pa)` / `pitcherDepthFromIp(p.ip)`) still reads the identity copies. It only fires when the
prediction's stored depth role is missing/invalid — which the two fixes above make rare — so it is logged, not
patched, to keep that change contained.

Track B is the daily run that should *contain* all of this.
