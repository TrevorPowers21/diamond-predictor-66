...
**READINESS:** this is a **BLOCKER**. Prod and staging compute hitter projections with DIFFERENT
EQUATIONS. No cross-environment verification is meaningful until the config sources are unified.
...
| H4 | HIGH | `backfill_park_factors_seasonal.ts:37` hardwires staging URL literal + `.env.local`; input CSVs live at uncommitted `~/RSTR IQ Data/park-factors` (not reproducible from repo). Prod producer (ledger:197). | Add env selection (`SUPABASE_URL \|\| VITE_SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY`) + `--prod` guard — 🛑 **copy the pattern from `scripts/load-drs-wsb-staging.ts:29-31` (explicit `trbvxuoliwrfowibatkm` ref assert both ways), NOT from `recompute-stuff-plus.ts`**, which is the banned LEGACY raw-HB Stuff+ lane and must not be opened or run; run with `--env-file .env.production.local`. Confirm park-factor CSVs present on push machine. | 2 | Park factors seasonal fill (ledger:197) |
...
- `team_season_stats`: RLS enabled with **no policy** → service-role-only (over-restrictive, **not a leak**). ✅ **CORRECTED 2026-08-30: it IS on prod now** (table + `refresh_team_season_stats`), 0 rows until F44. If a client reads it, add `FOR SELECT TO authenticated USING(true)` before shipping the reader.
...
- **Half-shipped / do not assume active:**
  ...
  - 🚨 **ELEVATED MEDIUM → BLOCKING FOR F41 (re-confirmed in code 2026-08-31)** — D1 TWP hitter market priced off `o_war` only (`rebake-twp-markets.ts:34,44,57`; ~~`clean-twp-sides.ts:26`~~ **🗑️ DELETED 2026-08-31**; `fix-returner-twp-hitter-market.ts:86`) vs `total_hitter_war` for non-TWP and the transfer path → same column priced on two WAR bases. **This was MEDIUM when logged; F39 + F40 (applied 2026-08-31) made `total_hitter_war` the canonical basis EVERYWHERE ELSE, so running F41 now actively creates the split rather than merely leaving it.** Measured on prod: Overbeek `o_war` 2.008 vs `total` 2.049 (−2%), Colasante 0.924 vs 1.021 (−10%). Make TWP re-bake price off `total_hitter_war`; TWP rows **do** carry `d_war`/`bsr_war` (F39 filled 201,221 rows). ⬜ **NEEDS TREVOR'S CALL.** (Ledger 463, 498.) See the **F41 PRE-FLIGHT AUDIT** block.
  - 🗑️ **LOW — MOOT: `rebake-twp-markets.ts` was DELETED 2026-08-31.** (For the record: the guard had been added and the unordered `.range()` was re-verified benign after `is_twp` 137 → 253 — both loops single-page, `team_build_players` read via chunked `.in()`. The open "writes unconditionally, no `abs(old−new) >= 1` gate" item dies with the script.) **Superseded by F42**, which writes the same two tables at the **build program's** conference; this one priced at the **player's** conference (−65% measured). ♻️ `git show 9c61e7d:scripts/rebake-twp-markets.ts`
