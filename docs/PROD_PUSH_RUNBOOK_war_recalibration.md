...
**RUNBOOK STEPS:** (1) snapshot current prod wRC+ values as a rollback reference · (2) rename
`"Equation Weights"` → `"Equation Weights_LEGACY_2025"` · (3) delete the `predictionEngine.ts:379-405`
override block + the dead model_config fallback · (4) settle the `p_` vs `_plus_` key convention ·
(5) apply calibration · (6) mirror the edge fn · (7) recompute · (8) verify across the range.
...
**RUNBOOK STEPS THIS ADDS (staging first, then prod):**
1. `npx tsx --env-file=<env> scripts/compute-projection-calibration.ts` — DRY RUN, confirm 41 keys and
   the `⚠ far from 100` flags on the 5 pitching centers.
2. Re-run with `--apply` to write `model_config`.
3. Mirror the fix into the **edge function** (its own constants + its own hardcoded `100`) — otherwise
   onboarding writes old-bias projections into a new program.
4. Re-run the precomputes (returner + transfer, hitters + pitchers).
5. Verify the across-the-range table before promoting.
...
|---|---|---|
| `team_season_stats` | `20260819000000_*` | ✅ CREATE **already applied on prod 2026-08-30** (table + fn exist, 0 rows). Populate via `refresh_team_season_stats(2026)` — 🔴 **NOT "LAST": it must run BEFORE Phase E**, which reads `team_season_stats.faced_*`. See `docs/AUDIT_dependency_order_vs_topic_order_2026_08_30.md` |
...
8. **`refresh_team_season_stats(2026)`** — 🔴 **ORDER CORRECTED 2026-08-30: NOT LAST. It must precede Phase E**, whose transfer precomputes read `team_season_stats.faced_stuff_plus`/`.faced_htp` and silently coerce a missing table to an empty Map. Its own prereqs are Phase D + **D33b lock-season** + E2 park + C28.  Historically documented as LAST; consumes all of the above + prod's own `team_war_snapshots` (2025 champions).
...
| S | Step | Where |
|---|---|---|
| 1 | 🛑 **SUPERSEDED VALUE + WRONG KEY NAME (corrected 2026-08-30).** The live prod key is **`p_whip_pr_sd` = 37.19844** (not `whip_pr_sd` = 37.13 — that key name returns ZERO ROWS). Historical: `whip_pr_sd` 24.59 → **37.13** (confirmed on latest ratings) | `pitchingEquations.ts:210` + add to `model_config` (edge fn reads it) |
| 2 | 🛑 **SUPERSEDED VALUE + WRONG KEY NAME (corrected 2026-08-30).** Live prod keys are **`r_obp_std_pr` / `t_obp_std_pr` = 31.89504** (not `obp_std_pr` = 32.41 — that bare name returns ZERO ROWS). Historical: `obp_std_pr` 28.89 → **32.41** — **returner AND transfer** (it's `StdDevOBPPowerRating` in the returner equation) | `model_config` `r_obp_std_pr` + `t_obp_std_pr`; code fallbacks |
