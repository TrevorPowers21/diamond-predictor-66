...
**PLAN IMPACT:** GATE B is SOLVED by this (see the Gate B section) — it was the config override, not a
dev-scale or stale-copy problem. Fixing it is steps 1-2 of the config consolidation.
...
**WHERE THIS SITS IN THE PLAN:** it belongs with the other math gates (ERA scalar / wRC+ audit) and
**before** onboarding verification — dry-running a new team against numbers we already know are biased
proves nothing. Order: apply calibration → re-run precomputes → verify range → then onboarding.
...
## ✅ STEP 7a/7b — E36 pitchers + E37 hitters — **DONE 2026-08-31** (re-run after the depth-role fix; tiers now REG-anchored) · ✅ **E38 transfers DONE 2026-08-31** (13 teams x ~14,270 rows · All-Americans 0 by design) · ▶️ **NEXT: F39 `refresh_composite_war()`** (`zsh scripts/_run_step2_all.sh --prod`) (read the Masters — hence STEP 2/4 first)
```
...
F41  rebuild-twp-target-rows --apply ONLY (DONE). rebake-twp-markets DELETED. fix-returner-twp-hitter-market SKIPPED (all-JUCO).
...
```
**GATE:** 0 snapshots with `o_war` but NULL `total_hitter_war`; ⚠ **NOT a threshold** — see "GATE CORRECTION (2026-08-31)". Assert the IDENTITY `market ÷ total_hitter_war == 25,000 × PTM(program conf) × PVM(pos)` and **≤3 distinct rates per conference**; 0 negative markets; re-dry-run 0. ($130k/win is merely 25,000×4.0×1.3 — the formula's own max, not a rule. **Always label `total` vs `$/win`.**)

## 🛑 STEP 10 — G46 edge-fn deploy — **REMOVED FROM THIS PUSH (2026-08-31)**
🛑🛑 **REMOVED FROM THIS PUSH 2026-08-31 (Trevor) — DO NOT DEPLOY.** Delivers NOTHING prod is missing (every branch fix already reached prod via the BATCH path; `precompute_jobs` = 0 pending / 0 running). 🔴 Still carries **REGISTRY #9** — depth roles from `players.pa`/`players.ip`, ~49 D1 players mis-tiered. → Moved to the **Track B feature branch**. ⚠ The onboarding trigger is still ARMED — see "G46 IS REMOVED FROM THIS PUSH". Historical instruction preserved below.
