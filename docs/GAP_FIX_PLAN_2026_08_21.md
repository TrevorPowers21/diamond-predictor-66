...
## GAP 3 — Codify 9a raw-rate assembly + 9f WRC_plus [✅ CODIFIED a960334 — staging re-verify is the only remaining gate]
...
**⚠️ REMAINING GATE (before prod):** a staging idempotent re-run comparing output vs backup `_confstats_backup_preassembly` (proves the inlined `team_conf` reproduces the original helper). COULD NOT run this session: no Supabase MCP connected + `supabase --linked` = **PROD** (`trbvxuoliwrfowibatkm`), and running the write there violates staging-first/prod-confirm. Run when a staging connection is available (re-link to `slrxowawbijbjrkozqlj` or MCP), OR fold verify into the Track-B edge fn's conf-stats stage. Env+ denominators (avg .2777/obp .3823/slg .4365/iso .1588) + cFIP 3.157 are hardcoded 2026 D1 (ncaa_averages) — re-read for a new season.
