...
# HTP PARK-FACTOR SWAP — NOT APPLIED (2026-08-21) [⚠ PARTIALLY SUPERSEDED — see "HTP FIX" below]
The 08-13 HTP swap `(100−wRC+)`→conference park factor was NEVER built IN CODE. All compute sites use `OPR + 1.25·(Stuff+−100) + 0.75·(100−wRC+)`; no `wrc_park` in code. Formula duplicated ~8 sites (precompute-pitchers.ts:174 feeds projections; TeamBuilder:826; TransferPortal:504; PitcherProfile ×3; PitcherPage:282; ConferenceStatsPage:162; PitchingConferenceStatsTable:82; playerRisk:875).
**CORRECTION (2026-08-21):** the claim "stored hitter_talent_plus reflects the old formula" was WRONG — the STORED `hitter_talent_plus` actually matched the PARK-SWAP formula 30/30 (Trevor hand-stored the swap). The bug was the CODE computing HTP LIVE (pre-swap), ignoring the stored value. See the fix below.

# ⭐ HTP FIX — how we did it (2026-08-21)
**Discovery:** HTP was computed LIVE in every reader (`precompute-pitchers`, the `process-precompute-jobs` edge fn — which even used bare `Overall_Power_Rating` as HTP!, TeamBuilder, TransferPortal, ConferenceStatsPage) using `100−wRC+`. But the STORED `hitter_talent_plus` matched the park-swap `OPR + 1.25(Stuff+−100) + 0.75(100−run_env_factor)` **30/30** (0/30 matched pre-swap) — proven by recomputing both and comparing. So the transfers ran on a live pre-swap HTP that DIDN'T match the stored/display value → inconsistent.
**Clarifications proven in DB:** OPR = `Overall_Power_Rating` (PA-weighted avg of hitters' `overall_power_rating`, 30/30) — NOT the `0.15/0.4/0.45` composite (`offensive_power_rating`, which was a redundant/empty display col). `run_env_factor` = simple avg of member teams' `rg_factor` (Park Factors by conference_id) — verified exact (SEC 94.20 / Ivy 98.92 / NEC 112.35).
**The fix:** (1) committed producer `scripts/derive_conf_opr_htp.ts` (idempotent) — fills `offensive_power_rating`=Overall_PR, confirms `run_env_factor`=conf-avg rg, stores canonical park-swap `hitter_talent_plus`; (2) wired ALL readers to READ the stored `hitter_talent_plus` (added it to `useConferenceStats` for TB/TransferPortal), deleted the live-compute (incl. `calcHitterTalentPlusFromConference`); (3) re-ran pitcher transfers on the stored HTP. Result: ONE canonical stored HTP, read (never computed) everywhere — consistent across transfers, TB, display. **Lesson:** when a stored value and a live-computed value disagree, prove which is right by recomputing both against the formula before assuming the store is stale.
