# ▶️ PROD PUSH — **THE NUMBERED STEPS**
> **ROLE OF THIS FILE:** the ordered, executable step list with exact commands. **Gates live in
> `docs/PROD_PUSH_BULLETPROOF_CHECKLIST.md`; the build spec lives in `PIPELINE_pitch_log_to_projections.md`.**
> 🛑 **THE NUMBERING IS A TABLE OF CONTENTS, NOT THE EXECUTION ORDER.** The real order is the read/write graph —
> that audit moved **F44 ahead of Phase E** and invented **D33b**. See `docs/AUDIT_dependency_order_vs_topic_order_2026_08_30.md`.
> **CURRENT STATE (2026-08-31): ALL PROD WRITES ARE COMPLETE.**
> `E36 · E37 · E38 · F39 · F40 · F41a · F42a/b/b′ · F43a/F43b` ✅ — all gates pass.
> 🛑 **G46 IS REMOVED FROM THIS PUSH** → Track B feature branch. ⚠ its onboarding trigger is still ARMED.
> ▶ **REMAINING: preview-verify → `gh pr create` staging→main → Trevor merges → Phase H drops.**
...
**ORDERING:** the config-source unification must precede ANY step that writes or verifies
projections. Applying calibration into a system with three disagreeing config sources just moves the
...
**STEPS THIS ADDS — run BEFORE any step that writes or verifies projections:**
`compute-projection-calibration.ts` dry-run → confirm 41 keys → `--apply` (staging, then prod) →
mirror the fix into the edge fn → re-run precomputes → verify the across-the-range table.
...
46. 🛑🛑 **REMOVED FROM THIS PUSH 2026-08-31 (Trevor) — DO NOT DEPLOY.** Delivers NOTHING prod is missing (every branch fix already reached prod via the BATCH path; `precompute_jobs` = 0 pending / 0 running). 🔴 Still carries **REGISTRY #9** — depth roles from `players.pa`/`players.ip`, ~49 D1 players mis-tiered. → Moved to the **Track B feature branch**. ⚠ The onboarding trigger is still ARMED — see "G46 IS REMOVED FROM THIS PUSH". Historical instruction preserved below.

    `supabase functions deploy process-precompute-jobs --project-ref trbvxuoliwrfowibatkm` — two-sided SD + HR9 floor + TWP-aware + per-conference PTM + faced-competition. Deploy AFTER prod has conf env+ / `ba/obp/iso_plus` + model_config transfer weights. (Staging is at v27; prod is v12.)
