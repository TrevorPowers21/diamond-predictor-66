...
### 11. team_season_stats (Program Analytics)
`refresh_team_season_stats(season)` → Σ Masters **descriptive** WAR (owar/dwar/bsrwar/pwar reg+total) + conf context (conf_stuff_plus/conf_htp/conf_opr from Conference Stats) + faced-competition + park + records. 308 D1 rows. Descriptive (actual season), separate from projections. ✅
