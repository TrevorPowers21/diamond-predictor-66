...
const { data: players } = await (sb as any)
  ...
  .ilike("last_name", "Jenkins")
  .limit(5);

if (!players || players.length === 0) { console.log("Sean Jenkins not found"); process.exit(0); }
for (const p of players) {
  console.log(`Sean Jenkins | id=${p.id} | team=${p.team} | pos=${p.position} | ip=${p.ip} | div=${p.division} | class=${p.class_year}`);
}
