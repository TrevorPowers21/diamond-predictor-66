...
(async () => {
  ...
  const pc = new Map<string, string>(), pt = new Map<string, string>(), psrc = new Map<string, string>(), ptw = new Set<string>();
  { let f = 0; for (;;) { const { data } = await sb.from("players").select("id,team_id,team,source_player_id,is_twp").order("id").range(f, f + 999); for (const p of (data || [])) { if (p.team_id && tc.get(String(p.team_id))) pc.set(p.id, tc.get(String(p.team_id))!); if (p.team) pt.set(p.id, p.team); if (p.source_player_id) psrc.set(p.id, String(p.source_player_id)); if (p.is_twp) ptw.add(p.id); } if (!data || data.length < 1000) break; f += 1000; } }
  ...
  let from = 0;
  for (;;) {
    const { data, error } = await sb.from("player_predictions").select("id, player_id, customer_team_id, pitcher_role, pitcher_depth_role, p_rv_plus, p_war, projected_ip, market_value, twp_pitcher_market_value").eq("season", 2027).not("pitcher_depth_role", "is", null).order("id").range(from, from + 999);
    ...
    for (const r of (data || [])) {
      ...
      if (tw) patch.twp_pitcher_market_value = (d as any).twp_pitcher_market_value; else patch.market_value = d.market_value;
